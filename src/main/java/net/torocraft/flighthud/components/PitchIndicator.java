...
public class PitchIndicator extends HudComponent {
    ...
    private final boolean doInvertedRoll = true; // Fix bars rolling the wrong way
    ...
    public void render(MatrixStack m, float partial, MinecraftClient mc) {
        ...
        if (CONFIG.pitchLadder_showRoll) {
            ...
            if (doInvertedRoll) {
                m.multiply(Vec3f.POSITIVE_Z.getDegreesQuaternion(-1 * roll)); // Fix bars rolling the wrong way
            } else {
